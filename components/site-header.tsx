...
export function SiteHeader() {
  ...
  return <>
    <header className={`header ${dark ? "header-dark" : ""}`}>
      ...
      <div className="header-actions"><SoundToggle /><KeyboardButton variant="space" size="md" className="header-cta" href={quoteHref} external label="Cotiza tu proyecto" /><KeyboardButton variant="normal" size="xs" className="menu-trigger" aria-label="Abrir menú" aria-expanded={open} onPress={() => setOpen(true)}><span className="menu-lines"><i /><i /></span></KeyboardButton></div>
    </header>
    <div className={`menu-overlay ${open ? "open" : ""}`} role="dialog" aria-modal="true" aria-label="Menú FORM4TH" aria-hidden={!open}>
      <KeyboardButton variant="normal" size="sm" className="menu-close" label="CERRAR" tabIndex={open ? 0 : -1} onPress={() => setOpen(false)} />
      <nav className="mobile-nav" aria-label="Navegación móvil">{links.map(([label, href], i) => <KeyboardButton key={href} variant="enter" size="lg" className="mobile-nav-key" href={href} label={`${String(i + 1).padStart(2, "0")} / ${label}`} tabIndex={open ? 0 : -1} />)}</nav>
      <div className="mobile-menu-bottom"><KeyboardButton variant="space" size="lg" className="mobile-quote" href={quoteHref} external label="Cotiza tu proyecto" tabIndex={open ? 0 : -1} /><div className="mobile-social">{social.map(([label, href]) => <KeyboardButton key={label} variant="normal" size="sm" className="mobile-social-key" href={href} external label={label} tabIndex={open ? 0 : -1} />)}<SoundToggle /></div></div>
    </div>
  </>;
}
