...
const links = [["Inicio", "/"], ["Servicios", "/servicios"], ["Proyectos", "/proyectos"], ["Nosotros", "/nosotros"]];
...
export function SiteHeader() {
  ...
  return <>
    <header className={`header ${dark ? "header-dark" : ""}`}>
      ...
      <a className="header-cta" href="https://wa.me/51912227953" target="_blank" rel="noopener noreferrer">Cotiza tu proyecto ↗</a>
    ...
    <div className={`menu-overlay ${open ? "open" : ""}`} role="dialog" aria-modal="true" aria-label="Menú FORM4TH" aria-hidden={!open}>
      ...
      <div><a className="button button-orange" tabIndex={open ? 0 : -1} href="https://wa.me/51912227953" target="_blank" rel="noopener noreferrer">Cotiza tu proyecto</a><div className="mobile-social">{social.map(([label, href]) => <a key={label} href={href} target="_blank" rel="noopener noreferrer" tabIndex={open ? 0 : -1}>{label}</a>)}</div></div>
    </div>
  </>;
}
